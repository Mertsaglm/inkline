...
- `gereksiz/` and `Proje Yardımcısı/` are gitignored, local-only, and not part of
  the app. Never add build-time dependencies on them.
...
`npx tsc --noEmit && npm run build` must both pass. The build succeeds without
any env vars (the app falls back to a setup screen), so a green build does not
prove the app is configured — check the browser too.
